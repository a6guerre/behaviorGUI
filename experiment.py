    ...
    def read_serial(self):
        ...
        for yo in out:
            ...
            if yo==b'\xfe' or yo==b'\xfd':
                ...
                self.difficulty = int.from_bytes(yo, byteorder='big')
                self.char_ladder = 13
            elif self.char_ladder == 13:
                self.char_ladder=0
                self.automatic=int.from_bytes(yo, byteorder='big')
                print('made it to 13')
                if self.return_lick:
                    self.return_lick=False
                    return (4,  self.LOR[self.direction], self.COR[self.correct], self.difficulty, self.automatic)
